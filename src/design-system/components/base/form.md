...
> 实现：Implemented<br>
> 验证：Pending<br>
> 来源：Sens.Design 表单组件 v2.1、`SensForm`、Form 相关基础组件；校验行为另见表单系统规范 v0.3 · `2160:77795`。<br>
> 预览：`/components/form`

## 组件边界

- 表单负责表单项布局、标题、选填文案、帮助入口、辅助说明、报错、字符限制和操作区骨架。
- 辅助说明、帮助、校验和空态的书写对照 `docs/agent-rules/copywriting.md`（句末无句号、全角标点、人称）；流程按钮对照 `consistency-flow-rules.md`。
- 必填只作为语义和校验规则，不在标题前展示红色星号；视觉层只展示选填项。
- 输入框、数字输入框、选择器、单选组、复选框组等具体控件仍回到各自基础组件规则。
- 配置栏表单、分步表单、锚点表单、模态级表单、带表格的表单、联动表单属于“业务样板间 > 表单”。提交后滚到第一项、分步跳出错步见复合 Form，不在基础 Form 定义滚动容器。卡片嵌套的**分组规则**见下文；完整图 1/2/3 结构样张可后续进样板间。
...
| 卡内水平对齐 | 卡片已有 `spacing/4x`（16px）内边距。卡内组标题与字段标题共用这条左缘。`group-content` 的 16 只服务灰条下内容，**不得**在卡内再套一层。上下布局时卡内控件仍跟自己的标题左对齐 |
...
- 内容区到页脚按钮区使用 `spacing/vertical/10x`（40）；操作区即使不在 `SensForm` 根节点内，该间距仍须成立。
- `SensFormActions alignWithControl` 仅作为特殊场景能力保留；基础表单默认不使用。

## 辅助信息与报错规则

- 辅助信息和字段报错共用同一个表单项信息槽。
- 表单项标题行与辅助文案遵循两档文字：标题 / 选填为 `font-size/m`（14px）与 `line-height/m`（22px），辅助文案为 `font-size/s`（12px）与 `line-height/s`（18px）。
- 标题与选填文案同一行，标题使用 Medium、选填使用 Regular，二者间距为 `spacing/vertical/1x`（4px）；辅助文案与标题行之间同样使用 4px。
- 辅助文案可放在所解释的字段、卡片或关联内容上方或下方；保持与所属内容最近，并允许跟随内容区宽度自然换行。
...
- 出现 / 消失时机见下文「表单校验」；不要把清错写成 Input / Select 的私有规则。
- 无论辅助文案位于上方或下方，与所属内容均使用 `spacing/vertical/1x`（4px）；不得因为位于卡片外而改用分组间距。

## 表单校验

> 来源：Sens.Design 表单系统规范 v0.3 · 2.1 表单校验 · `2160:77795`。校验闭环归 Form，不写成单个控件的私有规则。红框、框内 / 框外菱形、计数器变红见 `input.md` / `select.md`。文案公式见 `docs/agent-rules/copywriting.md`，本文不抄句子。页面级提交定位与分步跳转见 `composite/form.md`。

### 原则

- **及时**：尽早给出对错反馈。
- **就近**：报错贴在出错控件旁，不改用整页 Toast 代替字段报错。
- **非必要不报错**：避免频繁打断；能用防错（事先说明、选对控件）就不要先报错。

### 校验时机

| 类型 | 何时用 | 规则 |
| --- | --- | --- |
| 失焦校验 | **默认** | 字段被激活或填写后，焦点离开该控件的可交互区域时校验。覆盖漏填、重名、格式、长度等。 |
| 提交校验 | 整表提交，或用户从未激活的必填项 | 点提交后校验。可与失焦并存。复杂后端 / 跨字段逻辑走提交。 |
| 实时校验 | **例外** | 输入过程中反馈。优先失焦，避免边写边报。典型：字数上限截断继续输入、密码强度。 |
| 上传校验 | 选文件 / 上传后 | Form 只定触发时机；失败、超限、格式错误的列表 / 宫格视觉留给 Upload（组件未录入前不在 Form 展开）。 |

### 字段级报错

- 出错项标记警告 / 错误态；框下信息槽给出原因和修正方式。
- 有辅助说明时，报错**替换**辅助说明，不叠两行。
- Input、Select、日期等非输入项同样：容器红框 + 框下报错，不能只给输入框做。
- 容器宽度受限时，报错按控件区宽度折行，图标与首行文案横排，后续行与文案左缘对齐。
- 表单项之间仍是 `spacing/vertical/5x`（20），从信息槽底量到下一项标题。有辅助说明时替换同一槽，高度不变；无辅助说明时出现报错向下撑开，不遮盖下一项。

### 出现与消失

- 默认在失焦或提交时**出现**报错，不要一输入就报。
- 已报错时重新聚焦：保持报错，不自动改用户已输入内容。
- 用户开始修改（输入、清空、改选项）后立即去掉报错；失焦时再校验。合法则恢复正常 / 辅助信息；仍不合法则再次报错。
- 业务若需「改完仍留错，直到再次提交才清除」，必须在样板间单独声明，不能当默认。

### 防错

- 录入前在字段旁给出必要说明：字数、格式 / 大小、名称不可改等。
- 按场景选对控件，降低输错机会。
- 不把「必填未填就禁用提交」当作默认；提交按钮状态仍走 Button / 业务流程。
...
- 当前 `SensForm` 是布局与语义骨架，不承担原生 `<form>` 提交，也不内置校验运行时；真正的提交和失焦校验由业务表单层决定。预览 `/components/form` 用页面控制器演示：默认失焦才报、开始修改立即去掉、提交标出全部失败项并滚到第一项。

## 左右布局标题对齐

| 场景 | 标题对齐规则 | 组件对齐规则 |
| --- | --- | --- |
| 常规输入、选择器、数字输入等 | 标题区域按 `size/component-height/m` 形成 32px 对齐高度 | 标题对齐盒与 32px 控件在同一行垂直居中 |
...
- `labelAlign="control"` 适用于单个、单行且使用 32px 控件高度的字段。
- 字段含辅助文案、联动内容、嵌套卡片、多行 Radio / Checkbox、文本域或其他多行内容时，使用 `labelAlign="top"`；标题与控件区第一行内容对齐。
- **嵌套字段仍按自己的场景对齐**：外层因联动卡使用 `labelAlign="top"` 时，卡内左右布局的常规输入 / 选择器仍使用 32px 对齐盒并与控件垂直居中；卡内上下布局仍是控件跟自己的标题左对齐。外层 top 不得取消内层 32。
- 关联 Radio / Checkbox 组使用 `itemHeight="content"`，由组件自身完成图标与首行文字的对齐；调用方不得叠加页面级 padding、负 margin 或绝对定位补偿。
- Checkbox / Radio、图标与同一行主文案应视觉居中；具体尺寸、状态和组内间距仍遵循各基础组件规则。
...
| 8px | `spacing/vertical/2x` / `spacing/horizontal/2x` | 上下布局标题到控件；同一字段内选项到关联控件 / 关联卡片 / 关联结果；组件多排；控件与单位 / 后缀内容 |
...
注意：横向并列的独立组件区块使用 24px；同一字段内部的联动表达式、选项到关联卡使用 8px；图标、帮助入口、删除入口等贴近文字或内容时使用 4px。Radio / Checkbox 下面的关联卡必须包在**该字段内容区**，不能做成 Form 的下一个表单项（否则会吃到 20px 行距）。

## 单个录入组件尺寸

- 单个录入组件的宽度根据界面自由组合，不强制要求所有字段等宽。
- 表单项控件区最大宽度使用 `form/control/max-width`（当前 600），包括输入框、选择器、数字输入框、单选 / 复选组和纯文本说明。**通栏的是卡片本身**；嵌套在通栏卡片内时，限制的仍是控件，不是卡片宽度。取消某字段的 600 上限时，只作用于该字段的直接控件区（让卡片拉满）；卡内 Input / Select 不得被一起取消。
- 组件宽度不得小于对应基础组件最小值。
...
| 同一字段内选项到关联卡 | `spacing/vertical/2x` | Ready |
...
- 校验行为已按 Figma `2160:77795` 写入「表单校验」；页面级提交定位与分步跳转见复合 Form；清单见 `page-evaluation-checklist.md` §5.3.3。预览 `/components/form` 覆盖失焦 / 改字即清 / 提交定位，以及 Input 与 Select 同构报错。
