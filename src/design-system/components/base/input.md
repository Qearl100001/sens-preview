...
- 基础 Input 不包装占位符 Tooltip；长说明统一放到 `help` 文案。
...
当输入框右侧同时出现数字限制等自定义 `suffix` 与框内报错时，组件内顺序固定为：**报错菱形 → 数字**。数字限制只作为 `suffix` 内容传入，不由基础 Input 自动计算。

错误文案的长度与展示方式由表单层决定：空间受限时可只保留框内图标并通过 Tooltip 查看；空间足够时由 `SensFormItem` 在框外完整展示或按表单列宽换行。基础 Input 不测量文案长度，也不自动切换 `warningPlacement`。
...
> **antd 限制**：`genDisabledStyle` 禁用态边框仍会复用 `colorBorder`（16% 深分割线）。`SensInput` 已在 `input.css` 的窄作用域中用 `--sens-input-disabled-border` 覆盖为 8% 浅分割线；`components.Input.colorBorderDisabled` 继续作为 token / 预览板来源。
...
- 框内警告图标可进入键盘焦点，并提供“警告”可访问名称；Tooltip 文案由图标描述承接。
- 框外 `help` 生成稳定 `id`，并通过 `aria-describedby` 关联输入框；真实 Demo 使用 `role="alert"`。
- 状态矩阵是静态样张，`help` 不进入 live region，避免一次播报大量重复错误。
- `allowClear` 默认关闭；启用时默认使用 `useSensAllowClear()` 的 Sens 清空图标。展示页包含“已输入 + 清空 + 框内警告”真实验收场景。
